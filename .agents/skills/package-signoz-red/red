#!/usr/bin/env bun
// SigNoz launcher — a single-node SigNoz observability stack on Vultr.
//
...
//
// COLORS_PAR_PROFILE is the exception — signoz refuses to run when it is
...
// can only point signoz at another project's state.
//
...
// own: validation, the graph and the steps all live in the `package-signoz-red`
...
//
// "package-signoz-red" is null before this repository has been pushed and
...
// rewrites null to "github:getcolors/signoz#<sha>" in place.
//
...
const PINS = {
  "package-signoz-red": "github:getcolors/signoz#0bb4c0f322cd98d71bd03ef214e4b095f7e9fd56",
...
// add-deps. SIGNOZ_LIB_ROOT overrides everything with a working tree: point
// it at the `red/` directory of a signoz checkout whose dependencies are
 ...
 *
 * The payload's canonical home is skills/package-signoz-red/ inside the
 * signoz repository, whose working tree is the point: resolving a pinned
...
function checkoutRedDir() {
  ...
  return manifest?.name === "package-signoz-red-dev" ? candidate : null;
}
...
function installToCache() {
  ...
  const root = join(home, "package-signoz-red");
  ...
  writeFileSync(
    ...
    `${JSON.stringify({ name: "package-signoz-red-cache", private: true, dependencies: PINS }, null, 2)}\n`,
  );
...
let signoz;
try {
  signoz = await import("package-signoz-red");
} catch (err) {
  ...
  const libRoot = process.env.SIGNOZ_LIB_ROOT;
  ...
  if (libRoot) {
    ...
    signoz = await importWorkingTree(overrideDir, "SIGNOZ_LIB_ROOT");
    ...
    signoz = await importWorkingTree(redDir, "checkout");
  } else if (!PINS["package-signoz-red"]) {
    console.error(
      "red: this launcher carries no signoz pin.\n" +
        "It has not been stamped by `bb pin`, which cannot run until the " +
        "signoz repository has been pushed.\n" +
        "Point it at a checkout meanwhile: SIGNOZ_LIB_ROOT=/path/to/signoz",
    );
    process.exit(2);
  } else {
    ...
    // not to a path.
    signoz = await import(Bun.resolveSync("package-signoz-red", dir));
  }
}

export const run = signoz.run;
if (import.meta.main) await signoz.exec();
